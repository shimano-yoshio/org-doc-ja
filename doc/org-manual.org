...
Emacs calendar and diary.  
Org Org mode を
can be used 使うことができます
to implement 実装するために
many different project planning schemes, 
...
- {{{kbd(RET)}}} (~org-table-next-row~) ::
     ...
     still inserts a new line, so it それを
     can be used 使うことができます
...
#+pindex: constants.el
Also, properties (see [[*Properties and Columns]]) 
can be used 使うことができます
as constants 定数として
in table formulas: 表の数式の中の
for a property =Xyz= 
use 
the name =$PROP_Xyz=, and the property will be searched in the current outline
...
These three functions can be used 使うことができます
to implement 実装するために
associative arrays,
...
(see [[*Fast access to TODO states]]) 
to insert 挿入するために
these words 
into the buffer.  
...
the =y=, =w=, =m=, =d= and =h=
letters.  
If you need both a repeater and a special warning period in
...
- {{{kbd(C-c C-x C-d)}}} (~org-clock-display~) ::
     ...
     disappear 消えます
     when you change the buffer (see
...
The easiest way to create such templates
...
options オプションを
for the export commands.  For example:
...
the code 
is left as it is.  そのまま
When exporting to HTML, 
Org  Org-modeは
can use 使うことができます
either [[http://www.mathjax.org][MathJax]] 
（[[*Math formatting in HTML export]] を見てください）か
or transcode 
the math into images 
