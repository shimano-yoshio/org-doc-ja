...
and task management 
make use 使用します
of metadata which is part of an outline
node.  Based on this data, 
specific entries can be extracted in
...
place and their results can be captured in the file.  
This これは
...
Recent Emacs distributions include a packaging system which lets 可能にします
you install インストールすることを
Elisp libraries.  You can install Org with {{{kbd(M-x
...
reserved for 
users 
(see [[info:elisp::Key%20Binding%20Conventions]]).  
Here 
...
error occurred.  
Here これ
is how to produce 生成方法です:
a useful backtrace: 役に立つバックトレースを
...
Org Org-mode は
is based on Outline モードに基いていごとえ、
...
~org-cycle~, 
which is bound to the これは {{{kbd(TAB)}}} キーにバインドされています。
...
subtree, in order to structure the collapsed view.  
See  を見てください。
the variable 変数 ~org-cycle-separator-lines~ を
to modify this behavior. このふるまいを変更するには、
...
- {{{kbd(C-c C-x b)}}} (~org-tree-to-indirect-buffer~) ::
     ...
     tree.  If N is negative then go up that many levels.  
...
This これを
can be configured 
through the variable ~org-startup-folded~, 
...
Furthermore, さらに、
any entries with a =VISIBILITY= property (see [[*Properties
...
- {{{kbd(C-c C-j)}}} (~org-goto~) ::
     ...
     #+texinfo: @noindent
     See 見てください。
     also the 変数 ~org-goto-interface~ も
...
- {{{kbd(M-RET)}}} (~org-meta-return~) ::
     ...
     inserts a new heading at the end of the current subtree, 
     thus よって
     preserving its contents.  
     With a double {{{kbd(C-u C-u)}}}< prefix, 
     the new heading is created 
     at the end of the parent subtree 
     instead. かわりに
...
- {{{kbd(C-c C-x c)}}} (~org-clone-subtree-with-time-shift~) ::
     ...
     specify 
     if any timestamps in the entry should be shifted.  
     This これが
...
- {{{kbd(C-c ^)}}} (~org-sort~) ::
     ...
     supply your own function to extract the sorting key.  
     With a {{{kbd(C-u)}}} prefix, 
...
- {{{kbd(C-c /)}}} (~org-sparse-tree~) ::
     ...
     This これは
     prompts プロンプトを出します
     for an extra key to select a sparse-tree creating
     command.
...
hand-formatted lists can provide
...
- {{{kbd(M-S-LEFT)}}}, {{{kbd(M-S-RIGHT)}}} ::
     ...
     the new indentation would imply a different hierarchy.  
     To use 使うには
     the new hierarchy, 新しい階層構造を
...
- {{{kbd(C-c -)}}} ::
     ...
     the type of list, and its indentation.  
     With a numeric prefix argument N, 
     select the Nth bullet from this list.  If there is an
     ...
     are converted 
     to list items.  
     If the first line already was a list item, any item
...
text.  Markers always start with =fn:=.  例です:
...
See  the docstring of that variable その変数のドキュメント文字列を見てください。
for details. 詳細については、
...
- {{{kbd(C-c C-c)}}} ::
     ...
     with a prefix argument,  1 つの前置引数といっしょに
     offer the same menu 
     as {{{kbd(C-c C-x f)}}} と同じメニューを提供します。
...
A table 
is re-aligned automatically 
each time 
you press
{{{kbd(TAB)}}}, {{{kbd(RET)}}} or {{{kbd(C-c C-c)}}} 
inside the table.
{{{kbd(TAB)}}} 
also moves 
to the next field
...
- {{{kbd(RET)}}} (~org-table-next-row~) ::
     ...
     Creates 作成します。
     a new row if necessary.  必用であれば新しい行を
     At the beginning はじめか
     or end 
     of a line, 
     {{{kbd(RET)}}} が
     still inserts a new line, 改行を挿入するおで
     so it それを
     can be used 使うことができます
...
- {{{kbd(C-c `)}}} (~org-table-edit-field~) ::
     ...
     (see [[*Column Width and Alignment]] を見てください）
     When called with a {{{kbd(C-u)}}} prefix, just make
...
- {{{kbd(C-c |)}}} (~org-table-create-or-convert-from-region~) ::
     ...
     (see [[*Creation and conversion]] を見てください）。
...
- {{{kbd(M-x org-table-export)}}} ::
     ...
     The format 
     used 
     to export the file can be configured
...
using 
{{{kbd(TAB)}}} or {{{kbd(RET)}}}
---automatically re-aligns it.  If you want to disable
...
Furthermore, さらに
with some special setup, it is possible to maintain
tables in arbitrary syntax with Orgtbl mode.  
For example, 例えば
it is possible 可能です
to construct 
LaTeX tables with the underlying ease and power
...
Org 
knows 
the concept of a /column formula/ that will be applied to
...
To compute 計算するには
fields 
in the table from other fields, formulas must
reference other fields or ranges.  In Org, fields can be referenced by
name, by absolute coordinates, and by relative coordinates.  
To find 見つけだすには
...
separator lines, or "hlines".  Like with columns, 
you can use 使うことができます
...
format at least for the first field, i.e., すなわち
...
least as many rows or columns as the current table.  
Note 注意してください
that this これが
...
(see [[*Properties and Columns]] を見てください）
can be used 使うことができます
as constants 定数として
...
(see [[info:calc#Calling Calc from Your Programs][Calling Calc from Your Lisp Programs]] を見てください）
...
- =E= ::
     ...
     list contains only the non-empty fields.  
     With =E= 
     the empty fields 
     are kept.  For empty fields in ranges or empty field
     ...
     to use 0 を使うには
     instead  かわりに
     for both formula types.  For the value of
...
- =N= ::
     ...
     See 見てください
     the next section 
     to see how this is essential for computations with
...
(see [[info:calc#Logical%20Operations][Logical Operations]] を見てください）
For example 例えば
...
It is also possible to write a formula in Emacs Lisp.  
This can be これが
useful 役に立つことがあります
...
numbers, without quotes.  If you provide the =L= flag, 
all fields 
...
are displayed as
=HH:MM:SS= 
(see the first formula above).  （上の最初の数式を見てください）
With the =U= flag, 
seconds
are omitted 
...
to ~hours~ and displays the result as a fraction of hours 
(see the third formula 
...
use 使ってください
the formula editor 
(see [[*Editing and debugging formulas]] を見てください）
or edit the
...
rank results, group data, etc.  For practical
...
of a table.  When offering a formula for editing, 
Org Org-mode は
converts 変換します
references 
...
- {{{kbd(C-c ')}}} (~org-table-edit-formulas~) ::
     ...
     - {{{kbd(S-UP)}}}, {{{kbd(S-DOWN)}}}, {{{kbd(S-LEFT)}}}, {{{kbd(S-RIGHT)}}} ::
          ...
          Shift 
          the reference at point.  For example, 例えば
          if the reference
...
- =^= ::
     ...
     any formula 
     in the table may use =$m1= to refer to
...
- =$= ::
     ...
     For example, 例えば
     if a field in a =$= row contains =max=50=, then formulas
...
The Org file is scanned automatically for
...
(see [[*Activation]] を見てください）。
It stores a link to the
...
- /Org mode buffers/ ::
     ...
     of ~org-link-to-org-use-id~, create and/or use 
...
- /Other files/ ::
     ...
     string 
     (see [[*Search Options in File Links]] を見てください）
     pointing to the
     ...
     (see [[*Custom Searches]] を見てください）。

     You can also define dedicated links to other files.  
     See [[*Adding Hyperlink Types]] を見てください。
...
- {{{kbd(C-c C-l)}}} (~org-insert-link~) ::
  ...
  - /Completion support/ ::
       ...
       (see [[*Link Abbreviations]] を見てください）。
       If you
...
- {{{kbd(C-c C-o)}}} (~org-open-at-point~) ::
     ...
     When point ポイントが
     is on an internal link, this command runs the
...
See [[*Activation]] を見てください
for some advice。 いくつかのアドバイスについては
...
where we need to encode the URL parameter).  Using
...
As a degenerate case, a file link with an empty file name 
can be used 使うことができます
to search 検索するために
...
simply
mark 
any entry in a tree as being a TODO item.  In this way,
...
- {{{kbd(C-c C-t)}}} (~org-todo~) ::
     ...
     If TODO keywords have fast access keys 
     (see [[*Fast access to TODO states]] を見てください）、
     prompt for a TODO keyword 
     ...
     (see [[*Commands in the Agenda Buffer]] を見てください）。
...
- {{{kbd(S-RIGHT)}}} {{{kbd(S-LEFT)}}} ::
     ...
     Useful 役に立ちます
     mostly 主に
     if more than two TODO states are possible 
     (see [[*Extended Use of TODO Keywords]] を見てください）。
     See 見てください。
     also [[*Packages that conflict with Org mode]] も
     for a discussion 議論については
     f the interaction with
     ~shift-selection-mode~.  
     See 見てください。
     also the variable ~org-treat-S-cursor-todo-selection-as-state-change~.
...
- {{{kbd(M-x org-agenda t)}}} (~org-todo-list~) ::
     ...
     (see [[*Agenda Views]] を見てください）
     into
     a single buffer.  The new buffer is in Org Agenda mode, which
     provides 提供しています
     commands コマンドを
     to examine 確かめて
     and manipulate the TODO entries from
     the new buffer 
     (see [[*Commands in the Agenda Buffer]] を見てください）。
     See [[*The global TODO list]] を見てください。
     for more information. さらに情報については、
...
See 見てください。
the docstring ドキュメント文字列を
of the オプション ~org-todo-state-tags-triggers~ の
for details. 詳細については
...
Note 注意してください
that /tags// 
are another way to classify headlines in general and
...
then to =VERIFY=, and finally to =DONE= and
...
If you would like to quickly change 
an entry 
to an arbitrary TODO state 
instead かわりに
of cycling 切り替える
through the states, 
you can set up keys for
...
buffer and then use {{{kbd(M-TAB)}}} to complete it 
(see [[*Completion]] を見てください）。
...
dimmed font or even made invisible in agenda views 
(see [[*Agenda Views]] を見てください）。
...
(see [[*TODO keywords as workflow states][*Workflow states]] を見てください）。
...
2. The property =STYLE= is set to the value =habit= 
   (see [[*Properties and Columns]] を見てください）。
...
4. The TODO may also have minimum and maximum ranges specified 
   by using 使うことによって
   the syntax =.+2d/3d=, 
   which says that you want to do the task
   at least every three days, but at most every two days.

5. State logging for the DONE state is enabled 
   (see [[*Tracking TODO state changes]] を見てください）、
   in order for historical data to be represented in
...
- ~org-habit-show-habits-only-for-today~ ::
     ...
     only show habits in today's agenda view.  The
     default value is ~t~.
...
for sorting in the agenda 
(see [[*Weekly/daily agenda]] を見てください）; 
outside the
...
- {{{kbd(C-c \,)}}} (~org-priority~) ::
     ...
     {{{kbd(SPC)}}} を
     instead, かわりに
     the priority cookie, if one is set, is
...
- {{{kbd(S-UP)}}} (~org-priority-up~); {{{kbd(S-DOWN)}}} (~org-priority-down~) ::
     ...
     (see [[*Creating Timestamps]] を見てください）。
     See 見てください。
     also [[*Packages that conflict with Org mode]] も
...
- {{{kbd(C-c C-c)}}} (~org-toggle-checkbox~) ::
     ...
     With a double prefix argument, 
     set it to =[-]=, which is considered to be an
     intermediate state.
...
(see [[*Faces for TODO keywords]] を見てください）。
...
- {{{kbd(C-c C-q)}}} (~org-set-tags-command~) ::
     ...
     changes 
     (see [[*Basic TODO Functionality]] を見てください）。

- {{{kbd(C-c C-c)}}} (~org-set-tags-command~) ::

     #+kindex: C-c C-c
     When point ポイントが
     is in a headline, this does the same as {{{kbd(C-c
     C-q)}}}.
...
drawer 
(see [[*Drawers]] を見てください）
with the name =PROPERTIES=, which has to be
located right below a headline, and its planning line 
(see [[*Deadlines and Scheduling]] を見てください）
when applicable.  
...
include these states in a column view 
(see [[*Column View]] を見てください）,
 or to use
...
To create 作成するには
sparse trees 
and special lists with selection based on
properties, the same commands are used as for tag searches 
(see [[*Tag Searches]] を見てください）.
...
- {{{kbd(C-c / m)}}} or {{{kbd(C-c \)}}} (~org-match-sparse-tree~) ::
     ...
     #+kindex: C-c \
     #+findex: org-match-sparse-tree
     Create a sparse tree with all matching entries.  
     With a {{{kbd(C-u)}}} prefix argument, 
     ignore 無視します
...
- {{{kbd(M-x org-agenda M)}}} (~org-tags-view~) ::
     ...
     (see 
     the option ~org-tags-match-list-sublevels~).
...
- ~ARCHIVE~ ::
     ...
     (see [[*Moving a tree to an archive file]] を見てください）.
...
- {{{var(PROPERTY)}}} ::
     ...
     (see [[*Special Properties]] を見てください）。
...
highs produces an unrealistically wide result.  Instead, かわりに
=est+= は
adds
...
itself, 
i.e., すなわち
of the headline.  You probably always should start the
...
use a =columnview= dynamic block 
(see [[*Dynamic Blocks]] を見てください）。
  The frame of
...
- =:vlines= ::
     ...
     force 
     column groups to get vertical lines.
...
- =:skip-empty-rows= ::
     ...
     When non-~nil~,   ~nil~ でない値のときには
     skip 
     rows where the only non-empty specifier of
...
- =:format= ::
     ...
     Specify a column attribute 
     (see [[*Column attributes]] を見てください）
...
- {{{kbd(C-c .)}}} (~org-time-stamp~) ::
     ...
     When point ポイントが
     is at an existing timestamp in the buffer, the command is
     used to modify this timestamp instead かわりに
     of inserting 挿入する
     ...
     When called with a prefix argument,  1 つの前置引数といっしょに呼ばれるときんいっは
     use 
     the alternative format
     which contains date and time.  The default time can be rounded to
     multiples of 5 minutes.  
     See the オプション ~org-time-stamp-rounding-minutes~ を見てください。

     #+kindex: C-u C-u C-c .
     With two prefix arguments, 
     insert 
     an active timestamp with the
...
- {{{kbd(S-UP)}}} (~org-timestamp-up~), {{{kbd(S-DOWN)}}} (~org-timestamp-down~) ::
     ...
     Note 注意してください
     that if point is in a headline and not at a timestamp, these
     same keys modify the priority of an item 
     (see [[*Priorities]] を見てください）。
     The
...
- {{{kbd(C-c C-y)}}} (~org-evaluate-time-range~) ::
     ...
     and end.  
     With a prefix argument,   1 つの前置引数といっしょ
     insert 挿入します
     result 
...
that today is *June 13, 2006*.  Here is how
...
years.  
With a single plus or minus, 
the date is always relative to
today.  
...
date/time format does not /replace/ the default format.  
Instead, かわりに
it それは
...
- =SCHEDULED= ::
     ...
     compilation for /today/, until the entry is marked DONE, 
     i.e., すなわち
...
However, しかし
it それっは
shows the item on each day where the
S-exp entry matches.
...
- {{{kbd(C-c C-d)}}} (~org-deadline~) ::
     ...
     also
     remove 
     any existing deadline from the entry.  Depending on the
...
- {{{kbd(C-c C-s)}}} (~org-schedule~) ::
     ...
     When called with a prefix argument,  1 つの前置引数といっしょに呼ばれるときには
     also remove 
     the scheduling date from the entry.  Depending on the
...
With the =+1m= cookie, 
the date shift is always exactly one month.  So
...
clock[fn:73] is retrieved 
(see [[*Resolving idle time]] を見てください）
and you are
...
- {{{kbd(C-c C-x C-i)}}} (~org-clock-in~) ::
     ...
     (see [[*Effort Estimates]] を見てください）、
     the mode line displays the current clocking time
     against it[fn:74].  If the task is a repeating one 
     (see [[*Repeated tasks]] を見てください）、
...
The {{{kbd(l)}}} key may be used in the agenda 
(see [[*Weekly/daily agenda]] を見てください）
...
#+texinfo: @noindent
To use 使うには
a specific time range you could write[fn:80]
...
- {{{kbd(C)}}} ::
     ...
     #+kindex: C
     To cancel the clock altogether, use {{{kbd(C)}}}.  
     Note 注意してください
...
and a =COLUMNS= format that displays
these values together with clock sums
...
#+vindex: org-columns-default-format
or, 
even better, you can set up these values globally 
by customizing カスタマイズすることによって
...
To get 得るには
an overview 概要を
of the entire part 全体部分のん
of the day 日の
that is committed, コミットされている
you can set 
the option ~org-agenda-columns-add-appointments-to-effort-sum~.  The
...
Capture lets 可能にします
you quickly store 
notes with little interruption of your
...
- {{{kbd(M-x org-capture)}}} (~org-capture~) ::
     ...
     defined 
     (see [[*Capture templates]] を見てください）、
     it offers these templates for
...
- {{{kbd(C-c C-c)}}} (~org-capture-finalize~) ::
     ...
     your work without further distraction.  
...
the location from where you called the capture command.  
This これが
can be extremely useful 極度に役に立つことがあります
for deriving tasks from emails, for example.  例えば
You fill in 
the task definition, press {{{kbd(C-c C-c)}}} and Org returns
...
- properties ::
     ...
     - ~:time-prompt~ ::
          ...
          filling the template.  Without this property, capture uses 使います
...
a directory belonging to an outline node.  
Org Org-mode は
uses 使います
directories 
named
...
in which it
stores 保存する
information about the status of items in the feed, to avoid
...
- {{{kbd(C-c C-w)}}} (~org-refile~) ::
     ...
     See 見てください。
     the variable 変数 ~org-refile-targets~ を
     for details.  詳細については
...
---for agenda views
---certain subtrees
without moving them to a different file, 
...
By default, デフォルトで
the report 
ignores commented (see [[*Comment Lines]]) and
...
~org-agenda-files~ を
but the easiest way 
to maintain it is through the following commands
...
the dispatcher 
(see [[*The Agenda Dispatcher]]).  To restrict the agenda
...
blocks together, for example 例えば
the weekly agenda, 
the global TODO list
and a number of special tags matches.  
See [[*Custom Agenda Views]] を見てください。
...
#+texinfo: @noindent
After that, その後では
everything happens automatically.  All diary entries
...
- If the comparison value is enclosed in double-quotes /and/ angular
  ...
  for these days at 0:00 hours, i.e., すなわち
...
See [[*Property Inheritance]] を見てください。
for details. 詳細については
...
of the item and other important information.  
You can customize カスタマイズすることができます
...
One useful setup 
is to use ~org-agenda-max-entries~ locally in a custom
command.  
For example, 例えば
this custom command 
displays the next five
entries 
...
- {{{kbd(C-c C-x a)}}} (~org-agenda-toggle-archive-tag~) ::
     ...
     Toggle トグルします
     the archive tag 
     (see [[*Internal archiving]]) for the current
     headline.
...
- {{{kbd(S-RIGHT)}}} (~org-agenda-do-date-later~) ::
     ...
     by a year.  
     With a {{{kbd(C-u)}}} prefix, 
     change the time by one hour.  If you immediately repeat
...
Custom agenda commands 
serve 
two purposes: to store and quickly access
frequently used TODO and tags searches, and to create 
...
can be quite useful 極めて役に立つことがあります
to use column view 
...
environment.  
This causes the following issues: これは次の問題を起こします:
...
or emphasize text.  Or you may need to insert
...
change this convention.  For example, 例えば
when setting 
this variable 
to ~{}~, 
=a_b= 
...
Exceptions 
include 
scientific notes, which often require mathematical
symbols and the occasional formula.  LaTeX[fn:109] is widely used to
typeset scientific documents.  
Org mode は
supports サポートしています
...
(see [[*Previewing LaTeX fragments]]).
...
CDLaTeX mode 
is a minor mode that is normally used in combination with
...
nicely 
with indented text 
and in particular with plain list structure (see
...
are labeled 
by the line numbers 
from the code listing.  Otherwise
...
---e.g., 例えば
LaTeX equations, 
source code blocks.  Depending on the
...
is set 設定されているときには
to a non-~nil~ value,   ~nil~ でない値に
Org prompts in the minibuffer.  To switch back to
...
Dispatcher]]) using the =Insert template= command 
...
- ~'~ ::
     ...
     Toggle トグルします
     smart quotes (~org-export-with-smart-quotes~).  Depending
...
- ={{{property(PROPERTY-NAME)}}}=; ={{{property(PROPERTY-NAME, SEARCH OPTION)}}}= ::
     ...
     Options in File Links]]) refers to a remote entry, 
     use it それを
     instead. かわりに
...
See the variable 変数 ~org-ascii-links-to-notes~ を
for details. 詳細については
...
- =BEAMER_THEME= ::
     ...
     Use 使ってくだせいあい
     square brackets for options.  例です:
...
- =DESCRIPTION= ::
     ...
     Use ~org-latex-hyperref-template~ to
...
- =KEYWORDS= ::
     ...
     multiple =KEYWORDS= lines if necessary.  By default, デフォルトで
     =hyperref=
     inserts 
     =KEYWORDS= as metadata.  Use
...
back-end, it recognizes other LaTeX specific syntax
---for example, 例えば
=#+LATEX:= or =#+ATTR_LATEX:=.  
See [[*LaTeX Export]] を見てください。
for details. 詳細については
...
#+texinfo: @noindent
See 見てください
the variable 変数 ~org-html-doctype-alist~ を
for details.  詳細については
The default is
~"xhtml-strict"~.
...
to ~<a>~ or ~<img>~ tags.  This example shows
...
This example 
shows 
realignment to right, and adds ~alt~
...
The default 
is to use 
the [[http://www.mathjax.org][MathJax]],
...
- =sdepth:= ::
     ...
     ~org-export-headline-levels~, i.e., すなわち
...
The LaTeX エクスポートバックエンド has several additional keywords for
...
- =LATEX_CLASS= ::
     ...
     level mapping that the LaTeX エクスポートバックエンド needs.  The back-end
...
The LaTeX エクスポートバックエンド can insert any arbitrary LaTeX code, see
...
- =:mode= ::
     ...
     The LaTeX エクスポートバックエンド wraps the table differently depending
...
- =:math-prefix=, =:math-suffix=, =:math-arguments= ::
     ...
     The LaTeX エクスポートバックエンド inserts =:math-prefix= string value in
     a math environment before the table.  The LaTeX エクスポートバックエンド
     inserts =:math-suffix= string value in a math environment after
     the table.  The LaTeX エクスポートバックエンド inserts =:math-arguments=
...
Use 
the =placement= attribute to modify a floating environment's
placement.
...
(see [[*Advanced Export Configuration]]).  
To change 
this behavior 
to use
another environment 
...
To set
a limit 
to a level before the absolute limit (see [[*Export Settings]]).
...
- {{{kbd(C-c C-e o o)}}} (~org-export-to-odt~) ::
     ...
     is specified, 
     the ODT export back-end automatically converts 変換します
...
keywords for each additional command in the header, 
 for example: 例えば
...
The back-end inserts the contents within
...
in the Org file 
using 使って
the default command =@table=, which results in
...
true, which it is in this example, 
it creates 作成します
a box 箱を
around the code
...
the appropriate location in the linked file.  
For example, 例えば
...
can be configured as to which files to
...
Org expand =:noweb= references by default. デフォルトで
...
however, しかし
is the same 同じです
for all languages.  This includes declaring
...
You need to take them into account, even when =colnames= or
...
variable ~org-babel-inline-result-wrap~, which by default デフォルトで
is set 
to ~"=%s="~ 
...
blocks and =CALL= keyword.  It is useful 役に立ちます
for protection 
against
...
specifications 
(see [[*Environment of a Code Block]] を見てください）。
That makes results
...
(see [[*Evaluating Code Blocks]] を見てください）。
...
references 
(see [[*Noweb Reference Syntax]] を見てください）。
...
- =noweb= ::
     ...
     Includes =link= option, expands Noweb references 
     (see [[*Noweb Reference Syntax]] を見てください）、
...
of expansion 
by ~org-babel-expand-src-block~ also assigns values 
(see [[*Environment of a Code Block]] を見てください）
to variables.  Expansions also replace
...
- ~org-src-preserve-indentation~ ::
     ...
     during export or tangling, which これが
...
.  Unlike minibuffer completions, which
are useful 役に立つ
for quick command interactions, Org's in-buffer completions
...
- {{{kbd(M-TAB)}}} ::
     ...
     - After =:= in a headline, complete tags.  Org deduces the list
       of tags from the =TAGS= in-buffer option 
       (see [[*Setting Tags]] を見てください）、
     ...
     - When point ポイントが
       is anywhere else, complete dictionary words using
       Ispell.
...
more details, see the variable's docstring.  
With Speed Keys activated, 
{{{kbd(M-x org-speed-command-help)}}}, or {{{kbd(?)}}} 
when point is at the beginning of an Org headline, shows currently active
...
- /Source code blocks/ ::
     ...
     For example, 例えば
     here is how to execute ditaa code blocks without
     prompting:
...
(see [[*Summary of In-Buffer Settings]] を見てください）。
...
- =#+STARTUP:= ::
     ...
     intervals can be configured using these options 
     (see variables
     ~org-log-done~, ~org-log-note-clock-out~, 
     and ~org-log-repeat~).
...
footnotes sections 
(see [[*Creating Footnotes]] を見てください）
...
further based on their usage needs.  For example, 例えば
the normal {{{kbd(S-<cursor>)}}} 
for editing timestamp might be better with
{{{kbd(C-c .)}}} chord.
...
of the Org manual depending on the syntax at point.  
For example, 例えば
using 
it 
on a headline displays "Document Structure" section.
...
- =ecomplete.el= by Lars Magne Ingebrigtsen ::
     ...
     buffers 
     (see [[*The Orgtbl Minor Mode]] を見てください）、
     but instead かわりに
     ---after
...
by specifying 
the respective key as property =CRYPTKEY=, 
e.g.: 例えば
...
TODO states /sets/ 
(see [[*Setting up keywords for individual files]] を見てください）
...
by using 使うことによって
TRAMP 
(see [[info:tramp][TRAMP User Manual]] を見てください）、
...
Push プッシュは
 creates 作成します
a special Org file =agendas.org= with custom agenda views
...
(see [[*Hyperlinks]] を見てください）、
and aninterface for adding new link types.  The following example shows the
...
3. The rest of the file implements necessary variables and functions.
   ...
   The function ~org-insert-link~ can insert it back
...
which is bound to {{{kbd(C-c C-x x)}}} by default.  デフォルトで
For example, 例えば
{{{kbd(C-c C-x x c l o c k t a b l e RET)}}} は
inserts a table that
updates the work time 
(see [[*Clocking Work Time]] を見てください）。
...
(see [[*The Agenda Dispatcher]] を見てください）。
...
Org 
does not honor the default visibility state when first opening a file for the
...
Use {{{kbd(C-M-i)}}} or {{{kbd(ESC TAB)}}} instead. かわりに
...
this command, 
the links 
are automatically enclosed 
in double brackets, and you will be asked for
...
are using both ~org-log-done~ and state change logging.  
However, しかし
it
never prompts 
for two notes: if you have configured both, the state
...
[[https://www.ctan.org/pkg/listings][listings]] package or the [[https://www.ctan.org/pkg/minted][minted]] package.  
Refer to
...
while using line numbers for the links, 
which これが
...
[fn:125] For a less drastic behavior, consider using a select tag 
(see [[*Export Settings]]) 
instead. かわりに
